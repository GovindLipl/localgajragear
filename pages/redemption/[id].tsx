...
const RedemptionDetail = () => {
  ...
  const handleBankTransferRedemption = () => {
    if (window.confirm("Are you sure to transfer this redemption?")) {
      backendTransferRedemption({ redemptionid: id, transactionID: '42141214142', details: "This is details", status: "Success" }).then((result) => {
        if (!result.isError) {
          router.push('/redemption');
        }
      }).catch((err) => {
                ...
                {(redemptionInfo.paymentDate) ? (<><Row className='align-items-start card-body'>
                  ...
                  <Col xl={4} sm={4} xs={12}>
                    <div className="mb-1">
                      ...
                      <span className="text-muted f-14">{redemptionInfo.transferName}</span>
                    </div>
                  </Col>
                  <Col xl={4} sm={4} xs={12}>
                    <div className="mb-1">
